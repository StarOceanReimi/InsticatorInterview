...
@Component
public class QuestionLuceneSearchService implements InitializingBean, AutoCloseable {

	private static final Logger LOGGER = LoggerFactory.getLogger(QuestionLuceneSearchService.class);
	...
	/**
	 * 
	 * @param term term that use for searching
	 * @param editDistance usually 1 or 2
	 * @return
	 */
	@SuppressWarnings("unchecked")
	public Iterable<Question> fuzzySearch(String term, int editDistance) {
        ...
        QueryBuilder builder = manager.getSearchFactory().buildQueryBuilder().forEntity(Question.class).get();
        Query luceneQuery = builder.keyword().fuzzy().withEditDistanceUpTo(editDistance)
                .withPrefixLength(1)
                .onFields("title", "tags.name", "index.name", 
                		  "index.options.name", "column.name", 
                		  "column.options.name")
                .matching(term).createQuery();
        javax.persistence.Query jpaQuery = manager.createFullTextQuery(luceneQuery, Question.class);
